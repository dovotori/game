export default class {
  ...
  genererHeightMap() {
    ...
    for (var z = 0; z < this.dimensions[1]; z++) {
      for (var x = 0; x < this.dimensions[0]; x++) {
        ...
        // TEXTURE
        pixel[cptRVBA] = pixel[cptRVBA + 1] = pixel[cptRVBA + 2] =
          this.hauteurs[x][z] * 255
        pixel[cptRVBA + 3] = 255
        cptRVBA += 4
      }
    }
  ...
  genererNormalMap() {
    ...
    for (var z = 0; z < this.dimensions[1] - 1; z++) {
      for (var x = 0; x < this.dimensions[0] - 1; x++) {
        ...
        v3 = new Vec3(
          sizeX,
          this.hauteurs[x + 1][z + 1] * this.hauteurMax,
          sizeZ,
        )
    ...
    for (var z = 0; z < this.dimensions[1]; z++) {
      for (var x = 0; x < this.dimensions[0]; x++) {
        ...
        if (x == 0 && z == 0) {
          ...
          var normale1 =
            normales[(this.dimensions[0] - 1) * (this.dimensions[1] - 2) * 2]
          var normale2 =
            normales[
              (this.dimensions[0] - 1) * (this.dimensions[1] - 2) * 2 + 1
            ]
          ...
          var normale1 =
            normales[(z - 1) * ((this.dimensions[0] - 1) * 2) + x * 2]
          var normale2 =
            normales[(z - 1) * ((this.dimensions[0] - 1) * 2) + x * 2 - 1]
          var normale3 =
            normales[(z - 1) * ((this.dimensions[0] - 1) * 2) + x * 2 + 1]
          normale = normale1.plus(normale2)
          normale = normale.plus(normale3)
          ...
          var normale5 =
            normales[z * ((this.dimensions[0] - 1) * 2) + x * 2 - 1]
          var normale6 =
            normales[z * ((this.dimensions[0] - 1) * 2) + x * 2 - 2]
          normale = normale1.plus(normale2)
          normale = normale.plus(normale3)
  ...
  draw(camera, lampe, haveCameraView) {
    if (this.program.isReady() && this.textureSol[0].isReady()) {
      ...
      this.program.setLumiere(
        lampe.getPosition().get(),
        camera.getPosition().get(),
      )
  ...
  getTailleTerrain() {
    return [
      this.dimensions[0] * this.tailleCarre,
      this.dimensions[1] * this.tailleCarre,
    ]
  }
  getHauteurMax() {
