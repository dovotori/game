export default class CameraFPS extends Camera {
  ...
  onKeyboard(input) {
    ...
    if (input.getClavier(40) || input.getClavier(83)) {
      this.position = this.position.moins(this.forward.multiplierValeur(vitesse)); //on recule
    }
  ...
  updateToTerrain(input, terrain) {
    ...
    var hauteurSurTerrain = terrain.getHauteur(this.position.x, this.position.z) + this.tmpTaille;
