export default class {
  ...
  deplacement(vx, vy, tileSize, map, mapSize) {
    ...
    // pour y
    if (
      this.essaiY(this.position, vy, this.taille, tileSize, map, mapSize) == 1
    ) {
      this.position.y += vy
    } else {
      // affinage
  ...
  essaiX(vx, tileSize, map, mapSize) {
    ...
    if (vx > 0) {
      if (
        this.collisionDroite(copix, this.position.y, tileSize, map, mapSize) ==
        1
      ) {
        return 0
      } else {
      ...
      if (
        this.collisionGauche(copix, this.position.y, tileSize, map, mapSize) ==
        1
      ) {
        return 0
      } else {
  ...
  essaiY(vy, tileSize, map, mapSize) {
    ...
    if (vy > 0) {
      if (
        this.collisionHaute(this.position.x, copiy, tileSize, map, mapSize) == 1
      )
        return 0
      else return 1
    } else if (vy < 0) {
      if (
        this.collisionBasse(this.position.x, copiy, tileSize, map, mapSize) == 1
      )
        return 0
      else return 1
    } else {
