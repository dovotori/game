export default class {
  ...
  draw(camera, programBump, programSprite, objet) {
    if (this.isLoaded) {
      ...
      xmax = Math.floor(
        this.scrollPos[0] / this.tileSize[0] + this.scrollSize[0],
      )
      ymin = Math.floor(this.scrollPos[1] / this.tileSize[1])
      ymax = Math.floor(
        this.scrollPos[1] / this.tileSize[1] + this.scrollSize[1],
      )
      ...
      for (var y = ymin; y < ymax; y++) {
        for (var x = xmin; x < xmax; x++) {
          var xCentre =
            x * this.tileSize[0] - this.scrollPos[0] - this.decalageEcran[0]
          var yCentre =
            -y * this.tileSize[1] + this.scrollPos[1] + this.decalageEcran[1]
  ...
  update(comportement) {
    ...
    if (comportementPos.x < boxMinX)
      this.scrollPos[0] -= boxMinX - comportementPos.x
    if (comportementPos.x > boxMaxX)
      this.scrollPos[0] += comportementPos.x - boxMaxX
    if (comportementPos.y > boxMinY)
      this.scrollPos[1] += boxMinY - comportementPos.y
    if (comportementPos.y < boxMaxY)
      this.scrollPos[1] -= comportementPos.y - boxMaxY
    ...
    if (this.scrollPos[0] < 0) {
      ...
      this.scrollPos[0] =
        this.mapSize[0] * this.tileSize[0] -
        this.scrollSize[0] * this.tileSize[0]
    }
    if (this.scrollPos[1] < 0) {
      ...
      this.scrollPos[1] =
        ...
        this.scrollSize[1] * this.tileSize[1]
    }
  }
